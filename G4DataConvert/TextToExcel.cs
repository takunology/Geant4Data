...
namespace G4DataConvert
{
    public class TextToExcel
    {
        ...
        //Excelファイル作成
        public void MakeExcelFile(int DicCount)
        {
            try
            {
                string ExcelDir = $@"{Program.ReadDir}\EXCEL";
                Directory.CreateDirectory(ExcelDir); //Excel変換作業用ディレクトリ生成

                IWorkbook Book;
                string FullFilePath = $@"{ExcelDir}\{txtFileName[DicCount]}.xlsx";
        ...
        //テキストファイル読み込みと書き込み
        public void WriteData(int DicCount, ISheet Sheet)
        {
            try
            {
                //ファイルの行数を取得
                string[] TxtLines = File.ReadAllLines(txtFilePath[DicCount]);
                ...
                while(streamReader.EndOfStream == false)
                {
                    ...
                    else // データ部分はタブ区切り
                    {
                        ...
                        string[] Value = TmpChar.Split('\t');
                        //for(int row = 0; row < TxtLines.Length; row++)
                        {
                            for (int col = 0; col < Value.Length; col++)
                            {
                                if (col % 7 != 0) //最後はnullなので飛ばす
